...
io.on('connection',function(socket){
    ...
    let room = `chat message-${socket.handshake.query.chat_id}`;
    // let room = `chat message`;
    socket.on(room, function(msg){
        var json = JSON.parse(msg);
        let event_name = `chat message-${json.data.id}`;
        var sql = `INSERT INTO ad_chats (message,name,chat_id,user_id,user_photo,voice,created_at) VALUES ("${json.data.message}","${json.data.name}","${json.data.id}","${json.data.user_id}","${json.data.user_photo}","${json.data.voice}","${json.data.created_at}")`;
